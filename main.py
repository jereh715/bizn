...
async def step_4_inject_form_and_complete(log_queue, page, custom_email, custom_phone, custom_password):
    ...
    log_queue.put_nowait("[*] Locating registration password field container...")
    # Targets the password input context within the Vuetify form frame
    pass_input_locator = page.locator('form.v-form .passField input').first
    await pass_input_locator.wait_for(state="visible", timeout=10000)
    
    log_queue.put_nowait("[*] Clearing any natively autogenerated layout payload...")
    await pass_input_locator.fill("") # Clears default string
    
    log_queue.put_nowait("[*] Injecting custom registration password context...")
    await pass_input_locator.fill(custom_password)
    
    # Keep validation eye-toggle sequence active for UI logging confirmation visibility
    ...
    recovered_password = await page.locator('form.v-form .passField input').first.input_value()
